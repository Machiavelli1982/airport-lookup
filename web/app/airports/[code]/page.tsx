...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const baseFallback: Metadata = {
    ...
    title: `${code || "Airport"} — Runways, ILS & Frequencies (MSFS Reference)`,
    description: `Technical data for ${code}: ILS frequencies, runway lengths, and navaids for MSFS 2020/2024 flight planning.`,
    ...
    <main style={{ padding: 18, maxWidth: 720, margin: "0 auto", fontFamily: "system-ui" }}>
      ...
      <div style={{ display: "grid", gap: 14 }}>
        ...
        <Card title="Airport Details" subtitle="Full geographical and organizational metadata.">
          ...
          <KV k="GPS Coordinates" v={mapsUrl ? <a href={mapsUrl} target="_blank" style={{ color: "var(--foreground)", fontWeight: 700 }}>{fmtCoord(airport.latitude_deg)}, {fmtCoord(airport.longitude_deg)}</a> : "—"} />
        ...
        <Card title="Runways & ILS Approach Data" subtitle="Detailed dimensions and instrument landing frequencies for simulation use.">
          {runways.map((r: any) => {
            ...
            return (
              <div key={r.id} style={{ border: "1px solid var(--border)", borderRadius: 14, padding: 16, marginBottom: 12, background: "rgba(255,255,255,0.01)" }}>
                ...
                <div style={{ color: "var(--muted)", fontWeight: 600, fontSize: 14, lineHeight: "1.6" }}>
                  <strong>Length:</strong> {r.length_ft ? `${r.length_ft} ft / ${fmtMFromFt(r.length_ft)}` : "—"} <br />
                  <strong>Width:</strong> {r.width_ft ? `${r.width_ft} ft / ${fmtMFromFt(r.width_ft)}` : "—"} <br />
                ...
                {ils && (
                  <div style={{ marginTop: 14, padding: 14, background: "rgba(34,197,94,0.05)", borderRadius: 10, border: "1px solid rgba(34,197,94,0.15)" }}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
                      <div>
                        <div style={{ fontSize: 10, color: "var(--muted)", fontWeight: 800, textTransform: "uppercase" }}>ILS Frequency</div>
                        <div style={{ fontWeight: 800, fontFamily: "monospace", fontSize: 16 }}>{Number(ils.ils_freq).toFixed(2)} MHz</div>
                      </div>
                      <div>
                        <div style={{ fontSize: 10, color: "var(--muted)", fontWeight: 800, textTransform: "uppercase" }}>ILS Ident</div>
                        <div style={{ fontWeight: 800, fontFamily: "monospace", fontSize: 16 }}>{ils.ils_ident}</div>
                      </div>
                      <div>
                        <div style={{ fontSize: 10, color: "var(--muted)", fontWeight: 800, textTransform: "uppercase" }}>Localizer Course</div>
                        <div style={{ fontWeight: 800, fontFamily: "monospace", fontSize: 16 }}>{Number(ils.ils_course).toFixed(0)}°</div>
                      </div>
                    </div>
                  </div>
                )}
        ...
        <Card title="Nearby Navaids" subtitle="Ground-based navigation aids (VOR, NDB, DME).">
          {navaids.length > 0 ? navaids.map((n: any) => (
            <div key={n.id} style={{ border: "1px solid var(--border)", borderRadius: 14, padding: 14, marginBottom: 10, background: "rgba(255,255,255,0.01)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontWeight: 800, marginBottom: 4 }}>
                <span style={{ fontSize: 16 }}>{n.ident} · {n.type}</span>
